...
#pragma region Solver
	...
	void Solver::init() {
		...
		//cout << "the ref obj = " << aux.refObj << endl;
	...
	void Solver::treeSearch(Solution &sln, int depth) {
		if (sln.totalCost - aux.refObj < 0.05) { return; }
		timer = Timer(1800s);
		...
		while (!topkLeaves.empty()) {
				...
				if (vid > trace.top()) {		// 偏序搜索
					...
					if (totalCost >= 0) {	// 该节点库存可行
						...
						if (!isINF && totalCost < bestCost) {	// 记录全局最优值
							...
							// 找到论文最优解，直接退出
							if (sln.totalCost - aux.refObj < 0.05) { break; }
						}
						// 记录可扩展的叶子，不一定要topk
						if ((level + 1) == depth) {
			...
			// 找到论文最优解，直接退出
			if (bestCost - aux.refObj < 0.05) { break; }
	...
	void Solver::treeSearch1(Solution &sln, int depth) {
		if (sln.totalCost - aux.refObj < 0.05) { return; }
		Price bestCost = sln.totalCost, totalCost;
		List<ID> bestChange;	// 记录最优解的变化路径
		ID nodeNum = input.nodes_size(), periodNum = input.periodnum();
		...
		// Price：成本， List<ID>：为树搜索路径，最后一个元素为下一轮迭代起始节点
		int visitNum = periodNum * nodeNum - 1;
		List<pair<Price, List<ID>>> topkLeaves{ { 0,{ -1 } } }, infNodes;
		while (!topkLeaves.empty()) {
			...
			List<ID> lastChange(std::move(leaf.second));// 备份到达该节点的访问路径
			loadVisits(visits, lastChange, root);		// 加载该节点处的 visits 状态
			int level = 0, vid = (root >= 0) ? (root + 1) : 1; // 下次迭代从父结点的子节点开始
			stack<int> trace;
			trace.push(root);
			...
			while (!trace.empty()) {	// 扩展该节点
				...
				ID pid = vid / nodeNum, nid = vid % nodeNum;
				if (vid > trace.top() && visits[pid][nid]) {		// 偏序搜索
					// 删除结点的访问
					visits[pid][nid] = 1 - visits[pid][nid];
					bool isINF = callModel(sln, visits, totalCost, vid);
					if (totalCost >= 0) {	// 该节点库存可行
						...
						if (!isINF) {
							if (totalCost < bestCost) {
								...
								std::cout << "best cost now : " << bestCost << endl;
								//cout << "current best change : " << endl;
								//for (const auto &c : bestChange) {
								//	cout << c << "->";
								//}
							...
							if (infNodes.size() < aux.leafNum) {	// 叶子集合没满，直接加入
								infNodes.push_back(std::move(leaf));
								// 按目标函数值降序排列，方便从最后取出最好的解
								sort(infNodes.begin(), infNodes.end(), [](pair<Price, List<ID>> l, pair<Price, List<ID>> r) {return l.first > r.first; });
							}
							else if (totalCost < infNodes.begin()->first) {	// 第一个是最差的叶子，跟它比较来决定是否留下
								infNodes[0] = std::move(leaf);
								sort(infNodes.begin(), infNodes.end(), [](pair<Price, List<ID>> l, pair<Price, List<ID>> r) {return l.first > r.first; });
							}
						}
						++level; ++vid;	// 不剪枝，访问下一层节点
		...
		outVisits(visits, "verify : ");
		if (aux.refObj - bestCost < -0.05) {
			loadVisits(visits, bestChange, -1);	// 执行删除后的状态
			outVisits(visits, "After Delete : ");
			for (auto &node : infNodes) {
				int root = node.second.back(), pid = root / nodeNum, nid = root % nodeNum;
				node.second.pop_back();			// 去除重复父结点

				List<ID> lastChange(std::move(node.second));// 备份到达该节点的访问路径
				loadVisits(visits, lastChange, root);		// 加载该节点处的 visits 状态
				while (visits[++pid][nid]) { continue; }
				visits[pid][nid] = 1 - visits[pid][nid];
				callModel(sln, visits, totalCost, -1, false);
				if (totalCost >= 0 && totalCost<bestCost) {
					bestCost = totalCost; bestChange = lastChange;
					bestChange.push_back(pid*nodeNum + nid);
					std::cout << "best cost now : " << bestCost << endl;
					// 找到论文最优解，直接退出
					if (bestCost - aux.refObj < 0.05) { break; }
				}
